...
function BotAvatar({ size = 30 }) {
  const { RiveComponent, rive } = useRive({ src:"/robot.riv", autoplay:true, stateMachines:["State Machine 1"] });
...
function BotAvatarStatic({ size = 56 }) {
  const { RiveComponent } = useRive({ src:"/robot.riv", autoplay:true, stateMachines:["State Machine 1"] });
