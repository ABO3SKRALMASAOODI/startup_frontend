...
function BotAvatar({ size = 30 }) {
  const { RiveComponent, rive } = useRive({ src:"/hustler-bubble-bot.riv", autoplay:true, stateMachines:["State Machine 1"] });
...
function BotAvatarStatic({ size = 56 }) {
  const { RiveComponent } = useRive({ src:"/hustler-bubble-bot.riv", autoplay:true, stateMachines:["State Machine 1"] });
