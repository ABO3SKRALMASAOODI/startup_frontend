    ...
    <div style={S.layout}>
      ...
      <div style={S.chatPanel}>
        ...
        <div className="chat-window" style={S.chatWindow}>
          ...
          {/* ── Progress bar: only shown during code builds, not text-only replies ── */}
          {(isRunning || isRendering) && codeChanged && (() => {
          ...
          {/* ── Typing indicator: shown when agent is thinking/writing (before reply) ── */}
          {(isRunning) && (() => {
            const lastMsg = messages.length > 0 ? messages[messages.length - 1] : null;
            const agentAlreadyReplied = lastMsg && lastMsg.role === "assistant";
            if (agentAlreadyReplied) return null; // Building bar handles post-reply state
